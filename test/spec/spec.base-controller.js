<<<<<<< HEAD:test/spec/spec.base-controller.js
...
describe('Form Controller', () => {
  ...
  it('implements event emitter', () => {
    const form = new Controller({ template: 'index' });
  ...
  describe('requestHandler', () => {
    ...
    beforeEach(() => {
      form = new Controller({ template: 'index' });
...
>>>>>>> fix the lint:test/spec/spec.form.js
  ...
  describe('get', () => {
    ...
    beforeEach(() => {
      ...
      cb = sinon.stub();
      sinon.stub(Controller.prototype, 'getValues').yields(null, {});
      sinon.stub(Controller.prototype, 'getErrors').returns({});
      sinon.stub(Controller.prototype, 'render');
    });

    afterEach(() => {
      Controller.prototype.getValues.restore();
      Controller.prototype.getErrors.restore();
      Controller.prototype.render.restore();
    });
    ...
    it('sets values to req.form.values', () => {
      Controller.prototype.getValues.yields(null, { foo: 'bar' });
      form.get(req, res, cb);
    ...
    it('does not emit "complete" event if form has fields', () => {
      form = new Controller({ template: 'index', fields: { key: {} } });
      form.get(req, res, cb);
      form.emit.withArgs('complete').should.not.have.been.called;
    });
...
=======
  ...
  describe('post', () => {
    ...
    beforeEach(() => {
      ...
      res = {};
      sinon.stub(Controller.prototype, 'validate').yields(null);
      sinon.stub(Controller.prototype, 'setErrors');
      sinon.stub(Controller.prototype, 'saveValues').yields(null);
...
>>>>>>> fix the lint:test/spec/spec.form.js
    ...
    afterEach(() => {
      Controller.prototype.validate.restore();
    ...
    describe('invalid form-level validation', () => {
      ...
      beforeEach(() => {
        Controller.prototype.validate.yields({ field: 'invalid' });
      });
  ...
  describe('getNextStep', () => {
    ...
    beforeEach(done => {
      form = new Controller({ template: 'index', next: '/next-page' });
      req = request({
        params: {},
        body: { field: 'value' },
        flash: sinon.stub()
    ...
    describe('with forks, and _getForkTarget returns /fork', () => {
      beforeEach(() => {
        sinon.stub(Controller.prototype, '_getForkTarget').returns('/fork');
        req.form.options.forks = [];
      });

      afterEach(() => {
        Controller.prototype._getForkTarget.restore();
      });

      it('calls _getForkTarget if forks are present', () => {
        form.getNextStep(req, res);
        Controller.prototype._getForkTarget.should.have.been.calledOnce;
      });
  ...
  describe('getForkTarget', () => {
    ...
    beforeEach(done => {
      sinon.stub(Controller.prototype, '_getForkTarget');
      form = new Controller({ template: 'index', next: '/next-page' });
      form._configure(req, res, done);
    });
...
=======
  ...
  describe('errorHandler', () => {
    ...
    beforeEach(() => {
      err = new Controller.Error('field');
      form = new Controller({ template: 'index', next: '/success' });
      req = request({
        path: '/index',
        form: {
          values: { field: 'value' }
...
>>>>>>> fix the lint:test/spec/spec.form.js
    ...
    it('redirects to req.path if not all errors have a redirect value', () => {
      err = {
        'field-a': new Controller.Error('field-a'),
        'field-b': new Controller.Error('field-b', { redirect: '/exitpage' })
      };
      form.errorHandler(err, req, res);
      res.redirect.should.have.been.calledWith('/index');
    });
...
>>>>>>> fix the lint:test/spec/spec.form.js
    ...
    describe('dependent fields', () => {
      ...
      let form;
      let oldControlleratters;
      let req;
      let res;
      let cb;

      beforeEach(() => {
        oldControlleratters = _.clone(Controller.formatters);
        Controller.formatters = _.extend(Controller.formatters, {
          'boolean-force'(value) {
            let state;
            if (value === true || value === 'true') {
              state = true;
      ...
      afterEach(() => {
        Controller.formatters = oldControlleratters;
      });
